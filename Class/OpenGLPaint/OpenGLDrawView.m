...
//    [self setBrushColorWithRed:components[0] green:components[1] blue:components[2] alpha:1];

    if (lineColor == [UIColor greenColor]) {
    ...
    else if (lineColor == [UIColor purpleColor]) {
        [self setBrushColorWithRed:.5 green:0 blue:.5 alpha:1];
    }
    else if (lineColor == [UIColor cyanColor]) {
        [self setBrushColorWithRed:0 green:1 blue:1 alpha:1];
    }else if (lineColor == [UIColor yellowColor]) {
        [self setBrushColorWithRed:1 green:1 blue:0 alpha:1];
    }
}
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    // 如果需要点击出现圆点，可以注释这个if
    if (CGPointEqualToPoint(_location, _previousLocation)) {
        return;
    }
