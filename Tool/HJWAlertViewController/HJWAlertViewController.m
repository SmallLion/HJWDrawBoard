...
@interface HJWAlertAction ()

@property (copy, nonatomic) void(^actionHandler)(HJWAlertAction *action);

@end

@implementation HJWAlertAction

+ (instancetype)actionWithTitle:(NSString *)title handler:(void (^)(HJWAlertAction *action))handler {
    HJWAlertAction *instance = [HJWAlertAction new];
...
@interface HJWAlertViewController ()
{
    UIView *_shadowView;
    UIView *_contentView;
...
@implementation HJWAlertViewController

+ (instancetype)alertControllerWithTitle:(NSString *)title message:(NSString *)message {
    
    HJWAlertViewController *instance = [HJWAlertViewController new];
...
- (void)creatAllButtons {
    ...
    for (int i=0; i<self.actions.count; i++) {
        ...
        [btn addTarget:self action:@selector(didCliHJWButton:) forControlEvents:UIControlEventTouchUpInside];
...
#pragma mark - 事件响应
- (void)didCliHJWButton:(UIButton *)sender {
    HJWAlertAction *action = self.actions[sender.tag-10];
...
- (NSArray<HJWAlertAction *> *)actions {
